...
| `list_vaults` | — | List active mounted vaults and whether each has managed `AGENTS.md` instructions |
...
| `vault_context` / `get_vault_context` | `[vaultPath]` | Get mounted-vault summary: entity types, folders, recent notes, and managed `AGENTS.md` instructions |
...
The starter content no longer lives in the app repo. `src-tauri/src/vault/getting_started.rs` holds the public starter repo URL (`refactoringhq/tolaria-getting-started`), delegates the clone to the git backend, then normalizes Tolaria-managed hidden guidance and type scaffolding (`.laputa/agents/AGENTS.md`, `.laputa/agents/CLAUDE.md`, `type.md`, `note.md`) so fresh starter vaults pick up the current defaults even when the remote starter repo still carries a legacy copy or an older pre-`type:` `is_a`-era template. `.laputa/agents/AGENTS.md` stays the canonical vault guidance file; `.laputa/agents/CLAUDE.md` is a compatibility shim that imports it for Claude Code without duplicating the instructions, and the hidden `.laputa/` folder keeps managed guidance out of the normal note browser. Optional `.laputa/agents/GEMINI.md` guidance is created only by the explicit AI guidance restore action. Once a user edits usable managed guidance, including changing its frontmatter `type`, the status command treats it as custom guidance instead of broken; repair remains reserved for missing, empty, frontmatter-only, unreadable, or exact replaceable managed templates/stubs. The clone helper still accepts the legacy `LAPUTA_GETTING_STARTED_REPO_URL` environment override so older automation can continue to redirect the starter source during the transition.
...
| `config_seed.rs` | Maintains hidden vault AI guidance (`.laputa/agents/AGENTS.md`, `.laputa/agents/CLAUDE.md`, and optional `.laputa/agents/GEMINI.md` shims), migrates legacy root guidance and `config/agents.md`, and repairs missing root type scaffolding such as `type.md` and `note.md` |
...
| `create_empty_vault` | Create a git-backed vault, then seed hidden managed guidance plus root `type.md` and `note.md` defaults |
...
| `get_vault_ai_guidance_status` | Report whether hidden managed `AGENTS.md`, `CLAUDE.md`, and optional `GEMINI.md` guidance are managed, missing, broken, or custom |
