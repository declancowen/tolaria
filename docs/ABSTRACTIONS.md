...
Recording transcript blocks are Markdown-durable editor blocks, not audio assets. The renderer captures microphone audio in memory, encodes a temporary 16 kHz mono WAV payload, sends it through `transcribe_recorded_audio`, and discards the audio after the embedded Whisper runtime returns text. `src/utils/recordingTranscriptMarkdown.ts` stores only transcript text and block metadata in a fenced `tolaria-recording` block, so vault history never contains the source audio. App-wide dictation uses the same local model runtime, always copies the final transcript to the clipboard, and inserts into the focused editor/text target when one is available.
