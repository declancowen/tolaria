...
6. **Process lifecycle and vault guidance**: Stdio MCP servers in `mcp-server/index.js` must exit when their external client closes stdin, and the desktop-owned `ws-bridge.js` child must be stopped on vault deselection, vault switch, and app exit. MCP context must include hidden managed `.laputa/agents/AGENTS.md` instructions for every active mounted workspace when those files exist.
